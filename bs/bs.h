/*
 메모
 1. 디폴트값 width, height는 제대로 동작하지 않는다!
 2. 반드시 3번째 인자에 클래스 문자열을 넘겨주도록 한다!
...
-(BOOL)application:(UIApplication *)application didFinishLaunchingWithOptions:(NSDictionary *)launchOptions{
    ...
    __bs_SELF = self;
    idleTimeoutMinutes_ = 30; //30분 기본 Idle시간

    //초기화
    [bsIO onCreate];
    [bsIni onCreate];
    [bsNotification onCreate];
    //윈도우 객체 생성시 iOS7 상태바 관련 이슈 처리
...
#pragma mark - 유휴타이머 관련 
-(void)sendEvent:(UIEvent *)event {
    [super sendEvent:event];
    //유휴타이머가 동작중이면 초기화 시킨다.
    if( idleTimer_ ) {
